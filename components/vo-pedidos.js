class VoPedidos extends HTMLElement {
    constructor() {
        ...
        this.appendChild(document.getElementById('tpl-pedidos').content.cloneNode(true));
    }
    ...
    connectedCallback() {
        ...
        document.addEventListener('platillosActualizados', () => this.renderSelectPlatillos());
        this.querySelector('#btn-add-platillo-pedido').addEventListener('click', () => this.agregarPlatilloAlPedido());
        this.querySelector('#form-pedido').addEventListener('submit', (e) => { e.preventDefault(); this.guardarPedido(); });
        
        this.querySelector('#tabla-pedidos-temp').addEventListener('click', (e) => {
            if(e.target.classList.contains('btn-quitar-plat')) this.quitarPlatilloPedido(e.target.dataset.index);
        });
        this.querySelector('#tabla-historial-pedidos').addEventListener('click', (e) => {
            if(e.target.classList.contains('btn-eliminar-ped')) this.eliminarPedido(e.target.dataset.numero);
        });
        this.querySelector('#tabla-historial-pedidos').addEventListener('change', (e) => {
    ...
    renderSelectPlatillos() {
        this.querySelector('#ped-platillo').innerHTML = StorageHelper.get('vo_platillos')
            .map(p => `<option value="${p.codigo}">${p.nombre} - $${p.precio}</option>`).join('');
    }
    ...
    agregarPlatilloAlPedido() {
        ...
        if(!codigo || isNaN(cantidad) || cantidad <= 0) return alert("Seleccione platillo y cantidad válida.");
    ...
    verificarInventario() {
        ...
        for (let codigo in insumosNecesarios) {
            ...
            if (insumo.cantidad < insumosNecesarios[codigo]) {
                alert(`Falta: ${insumo.nombre}. Necesitas ${insumosNecesarios[codigo]}, pero solo tienes ${insumo.cantidad}.`);
                return false; 
            }
        }
    ...
    renderHistorial() {
        const pedidos = StorageHelper.get('vo_pedidos');
        this.querySelector('#tabla-historial-pedidos').innerHTML = pedidos.length === 0 
            ? '<tr><td colspan="6">No hay pedidos registrados.</td></tr>'
            : pedidos.map(p => {
                const colorStyle = p.estado === 'pendiente' ? 'background: #FFF3CD; color: #856404; border: 1px solid #FFEEBA;' :
                                   p.estado === 'preparacion' ? 'background: #CCE5FF; color: #004085; border: 1px solid #B8DAFF;' :
